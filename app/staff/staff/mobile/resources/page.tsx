...
export default async function MobileResourcesPage() {
  ...
  const [{ data: assignments }, { data: resources }, { data: tasks }] = await Promise.all([
    supabaseAdmin
      .from('planning_resource_assignments')
      .select('id, resource_asset_id, actual_resource_asset_id, task_id, planned_start_at, planned_end_at, status, assignment_kind, note, picked_up_at, returned_at')
      .eq('company_id', auth.membership.companyId)
    ...
    supabaseAdmin.from('resource_assets').select('id, name, status').eq('company_id', auth.membership.companyId).is('archived_at', null).order('name').limit(500),
    supabaseAdmin.from('tasks').select('id, title, scheduled_start, scheduled_end').eq('company_id', auth.membership.companyId).is('archived_at', null).order('scheduled_start').limit(100),
  ])
  ...
  return (
    <AppShell auth={auth} title="Mina resurser" subtitle={`Dagens resursansvar för ${staffProfile.full_name ?? 'personal'}.`} actions={<Link href="/staff/mobile/day" className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800">Min dag</Link>}>
      <div className="grid gap-5 lg:grid-cols-[1fr_0.7fr]">
        <section className="space-y-4">
            ...
            return (
              <section key={assignment.id} className="coordiqo-card p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-sm text-slate-500">{assignment.assignment_kind === 'extra' ? 'Extra resurs' : 'Planerad resurs'}</p>
          ...
          }) : <EmptyState title="Inga planerade resurser idag" description="Om du tar en resurs ändå kan du lägga till den som extra resurs här till höger." />}
        </section>

        <aside className="coordiqo-card p-5">
          <h2 className="text-lg font-semibold text-slate-950">Lägg till extra resurs</h2>
          <p className="mt-1 text-sm text-slate-500">Använd när du tog en cykel, bil, nyckel eller annan resurs som inte fanns i din planering.</p>
