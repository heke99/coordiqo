...
import { bulkUpdateShiftsAction, quickAbsenceAction } from '@/lib/platform/actions'
      ...
      <div className="space-y-5">
        {bulkRun ? (
          <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-5">
            ...
            {bulkItems?.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{bulkRunItems.map((item) => <div key={item.id} className="rounded-2xl bg-white/80 p-3 text-xs text-emerald-900"><b>{item.shift_date}</b> · {item.status}{item.conflict_summary ? <span className="block text-amber-700">{item.conflict_summary}</span> : null}{item.skipped_reason ? <span className="block text-red-700">{item.skipped_reason}</span> : null}</div>)}</div> : null}
          </section>
        ) : null}
          ...
          <aside className="space-y-5">
            ...
            <section className="coordiqo-card p-5">
              ...
              <div className="mt-4 space-y-2">
                {visiblePresets.slice(0, 8).map((preset) => <Link key={preset.id} href={`/schedule?preset=${preset.id}`} className="block rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm"><b>{preset.name}</b><span className="block text-xs text-slate-500">{timeLabel(preset.start_time)}–{timeLabel(preset.end_time)} · {preset.preset_scope === 'company' ? 'egen' : 'system'}</span></Link>)}
              </div>
